...
int check_function(const char *func_to_run, const char *data, int length, int expected)
{
...
char *test_functions()
{
    mu_assert(check_function("print_a_message", "Hello", 5, 0), "print_a_message failed.");
...
char *test_failures()
{
    mu_assert(check_function("fail_on_purpose", "Hello", 5, 1), "fail_on_purpose should fail.");
