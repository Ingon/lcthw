...
struct Connection *Database_init(const char *filename, int max_rows)
{
...
void Database_write(struct Connection *conn)
{
    ...
    if(rc != conn->db->max_rows) die("Failed to write database 2.", conn);
...
int main(int argc, char *argv[])
{
    ...
    if(action == 'c') {
        ...
        struct Connection *conn = Database_init(filename, max_rows);
